...
import { calculateRetentionDeposits } from "./retention-metrics";
  ...
  if (isRetentionAgent) {
    ...
    if (agentClients.length > 0) {
      ...
      if (accountIds.length > 0) {
        // Use the shared retention deposits calculation helper
        ...
        totalDeposits = depositMetrics.totalDeposits;
        totalDepositVolume = depositMetrics.totalDepositVolume;
        avgDepositAmount = depositMetrics.avgDepositAmount;
        depositRate = depositMetrics.depositRate;
      }
    }
  }
